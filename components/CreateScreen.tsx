...
export default function CreateScreen() {
  ...
  const fetchFixtures = async () => {
    ...
    try {
      ...
      const results = [...(data.yesterday || [])].filter(
        (f: Fixture) => f.homeScore !== null && f.awayScore !== null
      );
      setFixtures(results);
  ...
  const fetchPhoto = async () => {
    ...
    try {
      const query = selectedFixture.venue || `${selectedFixture.home} football`;
  ...
  const fetchCrests = async () => {
    ...
    const [home, away] = await Promise.all([
      fetchCrest(selectedFixture.home),
      fetchCrest(selectedFixture.away),
    ]);
    setCrests({ home, away });
  };

  const getAccent = () => selectedTemplate.id === 'photo' ? photoColor : '#00FF87';
  ...
  const downloadCard = async () => {
    ...
    try {
      ...
      ctx.fillText(selectedFixture.date, PAD, 230);
      ...
      ctx.fillText(formatTeamName(selectedFixture.home).toUpperCase(), homeX, crestY + crestSize/2 + 44);
      ctx.fillText(formatTeamName(selectedFixture.away).toUpperCase(), awayX, crestY + crestSize/2 + 44);

      // Score
      const scoreY = crestY + 80;
      ctx.font = '200px "Bebas Neue", sans-serif';
      ctx.textAlign = 'right';
      ctx.fillText(`${selectedFixture.homeScore}`, SIZE/2 - 50, scoreY);
      ctx.textAlign = 'left';
      ctx.fillText(`${selectedFixture.awayScore}`, SIZE/2 + 50, scoreY);
      ...
      link.download = `matchday-${selectedFixture.home}-vs-${selectedFixture.away}.png`;
    ...
    <div className="min-h-screen bg-[#0A0A0A]">
      ...
      <div className="relative px-5 pt-14 pb-2 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className={`text-white font-bold text-xl transition-all duration-300 ${scrolled ? 'opacity-0' : 'opacity-100'}`}>
            Create
          </span>
        </div>
        {step !== 'pick' && (
          ...
          {['pick', 'template', 'preview'].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                ...
                : ['pick','template','preview'].indexOf(step) > i ? 'bg-zinc-700 text-white'
                : 'bg-zinc-900 text-zinc-600'
              }`}>
                {i + 1}
              </div>
              {i < 2 && (
                <div className={`w-8 h-0.5 rounded ${['pick','template','preview'].indexOf(step) > i ? 'bg-[#00FF87]' : 'bg-zinc-800'}`} />
              )}
            </div>
          ))}
                  ...
                  {fixtures.map(fixture => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-zinc-500 text-xs">{fixture.date}</span>
                  ...
                  <div className="flex gap-4">
                    <div className="flex-1">
                      ...
                      <input type="color" value={gradientStart} onChange={e => setGradientStart(e.target.value)}
                        className="w-full h-12 rounded-xl cursor-pointer border-0" style={{ padding: '2px' }} />
                    </div>
                    <div className="flex-1">
          ...
          {step === 'preview' && selectedFixture && (
            <div>
              ...
              <div id="card-preview" className="relative aspect-square rounded-2xl overflow-hidden mb-4 bg-zinc-900">
                ...
                {selectedTemplate.id !== 'photo' && (
                  <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${gradientStart}, ${gradientEnd})` }} />
                )}
                {selectedTemplate.id === 'photo' && (
                ...
                <div className="absolute inset-0 flex flex-col p-4">
                  ...
                  <p className="text-white/55 text-[10px] mb-auto">{selectedFixture.date}</p>
                  ...
                  <div className="flex items-center justify-between mb-2 px-2">
                    <div className="flex flex-col items-center gap-1">
                      {crests.home ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center">
                          <span className="text-white text-[8px] font-bold">{selectedFixture.home.slice(0,3).toUpperCase()}</span>
                        </div>
                      )}
                      <span className="text-white font-black text-[10px] uppercase text-center leading-tight max-w-[60px]">
                        {formatTeamName(selectedFixture.home)}
                      </span>
                    </div>

                    <div className="flex flex-col items-center">
                      <div className="flex items-center gap-2">
                        <span className="text-white font-black text-4xl">{selectedFixture.homeScore}</span>
                        <span className="text-[8px] font-bold px-1.5 py-0.5 rounded-full"
                          style={{ color: accent, backgroundColor: 'rgba(0,0,0,0.4)' }}>FT</span>
                        <span className="text-white font-black text-4xl">{selectedFixture.awayScore}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-1">
                      {crests.away ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center">
                          <span className="text-white text-[8px] font-bold">{selectedFixture.away.slice(0,3).toUpperCase()}</span>
                        </div>
                      )}
                      <span className="text-white font-black text-[10px] uppercase text-center leading-tight max-w-[60px]">
                        {formatTeamName(selectedFixture.away)}
                      </span>
                    </div>
                  </div>
